...
// versions that always NUL-terminate.
	...
	void FXSYS_snprintf(
		char* str, size_t size, _Printf_format_string_ const char* fmt, ...);
...
#ifdef _NATIVE_WCHAR_T_DEFINED
#define FXSYS_wcsicmp(str1, str2) \
			_wcsicmp((wchar_t*)(str1), (wchar_t*)(str2))
...
#define FXSYS_GetModuleFileName GetModuleFileName
	...
	int FXSYS_WideCharToMultiByte(
		uint32_t codepage, uint32_t dwFlags, const wchar_t* wstr, int wlen, char* buf,
		int buflen, const char* default_str, int* pUseDefault);
	int FXSYS_MultiByteToWideChar(
		uint32_t codepage, uint32_t dwFlags, const char* bstr, int blen, wchar_t* buf,
		int buflen);
	uint32_t FXSYS_GetFullPathName(
		const char* filename, uint32_t buflen, char* buf, char** filepart);
