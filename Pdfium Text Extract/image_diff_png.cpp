...
namespace image_diff_png {
	...
	namespace {
		...
		void ConvertBetweenBGRAandRGBA(
			const unsigned char* input, int pixel_width, unsigned char* output,
			bool* is_opaque) {
		...
		void ConvertRGBAtoRGB(
			const unsigned char* rgba, int pixel_width, unsigned char* rgb,
			bool* is_opaque) {
			for (int x = 0; x < pixel_width; x++) {
	...
	namespace {
		...
		public:
			...
			PngDecoderState(ColorFormat ofmt, std::vector<unsigned char>* o) :
				output_format(ofmt), output_channels(0), is_opaque(true), output(o),
				row_converter(NULL), width(0), height(0), done(false) {}
			...
			void (*row_converter)(
				const unsigned char* in, int w, unsigned char* out, bool* is_opaque);
		...
		void ConvertRGBtoRGBA(
			const unsigned char* rgb, int pixel_width, unsigned char* rgba,
			bool* is_opaque) {
			for (int x = 0; x < pixel_width; x++) {
		...
		void ConvertRGBtoBGRA(
			const unsigned char* rgb, int pixel_width, unsigned char* bgra,
			bool* is_opaque) {
			for (int x = 0; x < pixel_width; x++) {
				const unsigned char* pixel_in = &rgb[x * 3];
		...
		void DecodeInfoCallback(png_struct* png_ptr, png_info* info_ptr) {
			...
			png_uint_32 w, h;
			png_get_IHDR(
				png_ptr, info_ptr, &w, &h, &bit_depth, &color_type, &interlace_type,
		...
		void DecodeRowCallback(
			png_struct* png_ptr, png_byte* new_row, png_uint_32 row_num, int pass) {
			PngDecoderState* state =
				static_cast<PngDecoderState*>(png_get_progressive_ptr(png_ptr));
			...
			unsigned char* dest =
				&base[state->width * state->output_channels * row_num];
		...
		bool BuildPNGStruct(
			const unsigned char* input, size_t input_size, png_struct** png_ptr,
			png_info** info_ptr) {
			if (input_size < 8) return false; // Input data too small to be a png
	...
	// static
	bool Decode(
		const unsigned char* input, size_t input_size, ColorFormat format,
		std::vector<unsigned char>* output, int* w, int* h) {
		png_struct* png_ptr = NULL;
		png_info* info_ptr = NULL;
		if (!BuildPNGStruct(input, input_size, &png_ptr, &info_ptr)) return false;
		...
		png_set_progressive_read_fn(
			...
			&DecodeEndCallback);
		png_process_data(
			png_ptr, info_ptr, const_cast<unsigned char*>(input), input_size);
	...
	namespace {
		...
		void
			EncoderWriteCallback(png_structp png, png_bytep data, png_size_t size) {
		...
		void ConvertBGRAtoRGB(
			const unsigned char* bgra, int pixel_width, unsigned char* rgb,
			bool* is_opaque) {
			for (int x = 0; x < pixel_width; x++) {
		...
		public:
			explicit CommentWriter(const std::vector<Comment>& comments) :
				...
				for (size_t i = 0; i < comments.size(); ++i) AddComment(i, comments[i]);
			}
		...
		private:
			void AddComment(size_t pos, const Comment& comment) {
				...
				if (comment.key.length() > 79) return;
		...
		bool DoLibpngWrite(
			png_struct* png_ptr, png_info* info_ptr, PngEncoderState* state,
			int width, int height, int row_byte_width, const unsigned char* input,
			int compression_level, int png_output_color_type,
			int output_color_components, FormatConverter converter,
			...
			png_set_IHDR(
				png_ptr, info_ptr, width, height, 8, png_output_color_type,
			...
			if (comment_writer.HasComments()) {
				png_set_text(
					png_ptr, info_ptr, comment_writer.get_png_text(),
					comment_writer.size());
			}
#endif
			...
			if (!converter) {
				...
				for (int y = 0; y < height; y++) {
					png_write_row(
						png_ptr, const_cast<unsigned char*>(&input[y * row_byte_width]));
				}
			}
			else {
	...
	// static
	bool EncodeWithCompressionLevel(
		const unsigned char* input, ColorFormat format, const int width,
		const int height, int row_byte_width, bool discard_transparency,
		const std::vector<Comment>& comments, int compression_level,
		std::vector<unsigned char>* output) {
		...
		if (input_color_components * width < row_byte_width) return false;
		...
		bool success = DoLibpngWrite(
			png_ptr, info_ptr, &state, width, height, row_byte_width, input,
			compression_level, png_output_color_type, output_color_components,
			converter, comments);
	...
	bool Encode(
		const unsigned char* input, ColorFormat format, const int width,
		const int height, int row_byte_width, bool discard_transparency,
		const std::vector<Comment>& comments, std::vector<unsigned char>* output) {
		return EncodeWithCompressionLevel(
			input, format, width, height, row_byte_width, discard_transparency,
	...
	bool EncodeRGBAPNG(
		const unsigned char* input, int width, int height, int row_byte_width,
		std::vector<unsigned char>* output) {
		return Encode(
			input, FORMAT_RGBA, width, height, row_byte_width, false,
	...
	bool EncodeBGRAPNG(
		...
		return Encode(
			input, FORMAT_BGRA, width, height, row_byte_width, discard_transparency,
			std::vector<Comment>(), output);
	}
