...
namespace {
	...
	std::string GetFullPathForSnapshotFile(
		const std::string& exe_path, const std::string& bin_dir,
		const std::string& filename) {
		std::string result;
		if (!bin_dir.empty()) {
			result = bin_dir;
			if (*bin_dir.rbegin() != PATH_SEPARATOR) { result += PATH_SEPARATOR; }
		}
	...
	bool GetExternalData(
		const std::string& exe_path, const std::string& bin_dir,
		const std::string& filename, v8::StartupData* result_data) {
	...
	void InitializeV8Common(const char* exe_path, v8::Platform** platform) {
		...
		*platform = v8::platform::CreateDefaultPlatform();
		...
		v8::V8::SetFlagsFromString(
			predictable_flag, static_cast<int>(strlen(predictable_flag)));
	}
#endif // PDF_ENABLE_V8
...
std::wstring GetPlatformWString(FPDF_WIDESTRING wstr) {
	...
	size_t characters = 0;
	while (wstr[characters]) ++characters;
...
#ifdef PDF_ENABLE_V8
#ifdef V8_USE_EXTERNAL_STARTUP_DATA
bool InitializeV8ForPDFium(
	const std::string& exe_path, const std::string& bin_dir,
	v8::StartupData* natives_blob, v8::StartupData* snapshot_blob,
...
bool InitializeV8ForPDFium(
	const std::string& exe_path, v8::Platform** platform) {
	InitializeV8Common(exe_path.c_str(), platform);
	return true;
}
#endif // V8_USE_EXTERNAL_STARTUP_DATA
...
DocLoader::DocLoader(const char* pBuf, size_t len) : m_pBuf(pBuf), m_Len(len) {}

// static
int DocLoader::GetBlock(
	void* param, unsigned long pos, unsigned char* pBuf, unsigned long size) {
	DocLoader* pLoader = static_cast<DocLoader*>(param);
...
void DocSaver::ClearString() {
	m_String.clear();
}

// static
int DocSaver::WriteBlockCallback(
	FPDF_FILEWRITE* pFileWrite, const void* data, unsigned long size) {
	DocSaver* pThis = static_cast<DocSaver*>(pFileWrite);
