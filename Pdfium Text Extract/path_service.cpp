...
bool PathService::GetExecutableDir(std::string* path) {
	...
	if (GetModuleFileNameA(NULL, path_buffer, MAX_PATH) == 0) return false;
	*path = std::string(path_buffer);
	...
	if (_NSGetExecutablePath(&((*path)[0]), &path_length)) return false;
#else // Linux
...
bool PathService::GetSourceDir(std::string* path) {
	if (!GetExecutableDir(path)) return false;

	if (!EndsWithSeparator(*path)) path->push_back(PATH_SEPARATOR);
	path->append("..");
...
bool PathService::GetTestDataDir(std::string* path) {
	if (!GetSourceDir(path)) return false;
...
// static
bool PathService::GetTestFilePath(
	const std::string& file_name, std::string* path) {
	if (!GetTestDataDir(path)) return false;
