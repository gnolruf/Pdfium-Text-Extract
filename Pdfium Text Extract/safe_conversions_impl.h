...
namespace pdfium {
	namespace base {
		namespace internal {
			...
			struct StaticDstRangeRelationToSrcRange<
				Dst, Src, INTEGER_REPRESENTATION_SIGNED,
				INTEGER_REPRESENTATION_UNSIGNED> {
				static const NumericRangeRepresentation value =
					MaxExponent<Dst>::value > MaxExponent<Src>::value
					? NUMERIC_RANGE_CONTAINED
			...
			template<typename Dst, typename Src>
			struct StaticDstRangeRelationToSrcRange<
				Dst, Src, INTEGER_REPRESENTATION_UNSIGNED,
				INTEGER_REPRESENTATION_SIGNED> {
				static const NumericRangeRepresentation value =
					NUMERIC_RANGE_NOT_CONTAINED;
			};
			...
			inline RangeConstraint GetRangeConstraint(int integer_range_constraint) {
				assert(
					integer_range_constraint >= RANGE_VALID &&
			...
			inline RangeConstraint
				GetRangeConstraint(bool is_in_upper_bound, bool is_in_lower_bound) {
				return GetRangeConstraint(
					(is_in_upper_bound ? 0 : RANGE_OVERFLOW) |
			...
			template<
				typename Dst, typename Src, IntegerRepresentation DstSign,
			...
			template<typename Dst, typename Src>
			struct DstRangeRelationToSrcRangeImpl<
				Dst, Src, INTEGER_REPRESENTATION_SIGNED, INTEGER_REPRESENTATION_SIGNED,
				NUMERIC_RANGE_NOT_CONTAINED> {
			...
			struct DstRangeRelationToSrcRangeImpl<
				...
				static RangeConstraint Check(Src value) {
					return GetRangeConstraint(
						value <= (std::numeric_limits<Dst>::max)(), true);
				}
			};
			...
			inline RangeConstraint DstRangeRelationToSrcRange(Src value) {
				...
				COMPILE_ASSERT(
					std::numeric_limits<Dst>::is_specialized, result_must_be_numeric);
